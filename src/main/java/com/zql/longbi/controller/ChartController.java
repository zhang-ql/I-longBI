...
import com.zql.longbi.bimq.BiMessageProducer;
...
//                "{明确的数据分析结论、越详细越好，不要生成多余的注释}";
    ...
    @PostMapping("gen/async/mq1")
    public BaseResponse<BiResponse> genChartByAiAsyncMq1(@RequestPart("file") MultipartFile multipartFile,
                                                         GenChartByAiRequest genChartByAiRequest, HttpServletRequest request) {
        String name = genChartByAiRequest.getName();
        String chartType = genChartByAiRequest.getChartType();
        String goal = genChartByAiRequest.getGoal();
        ...
        ThrowUtils.throwIf(!validFileSuffixList.contains(FileUtil.getSuffix(originalFilename)), ErrorCode.PARAMS_ERROR, "后缀名不对");
        User loginUser = userService.getLoginUser(request);
        redisLimiterManager.doRateLimit("genChartByAi_" + loginUser.getId());
